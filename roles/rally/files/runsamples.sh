...
for task in doc/samples/tasks/nova/*; do
    ...
    openstack-rally task results $tuuid |tee ../devstack/logs/rally-$task-results |jq . || die "invalid jason returned by $task: $(cat $tmptask)"
    openstack-rally task detailed $tuuid |tee ../devstack/logs/rally-$task-detailed | grep -q finished || die "Task failed: $task: $(cat $tmptask)"
done
